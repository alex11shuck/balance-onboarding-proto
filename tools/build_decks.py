...
BADGES = [{"l":"App of the Day","s":"App Store, [N] countries"},{"l":"Best of [2021]","s":"Google Play"},{"stars":True,"l":"4.8","s":"[180k+] ratings"}]
REVIEWS = [
...
add(id="welcome", type="welcome", phase=1, title=["Less stress. Better sleep.","Adapted to <b>you</b>."],
    authority={"avatars":True,"text":"Guided by meditation teachers Ofosu and Leah. [Built with clinical advisors.]"}, laurels=BADGES, cta="Continue",
    ...
    notes=N("welcome (remote config)","copy","Proof on screen 1: coach faces, an authority line, three plain badges. Headline is the variant winning the live welcome-card test (b_benefit). The welcome card is payload-configurable (title, CTA, image, laurels), so this is config, not a release.",
      calmer="#1 (welcome: 'built in collaboration with clinical psychologists')", evidence="Alex 1:40; ABCD test b_benefit; March A3 (store listing is the strongest pre-app trust signal)",
      fills=["Authority line: confirm the clinical-advisor claim with Anna or Cindy","App of the Day country count","Google Play award and year","Rating and ratings count from the App Store listing"]))
...
add(id="gender", type="question", phase=2, questionId="gender", title=["How do you identify?"], subtitle="Optional. Some members prefer sessions that speak to their experience.", reassure="Optional, and never shown anywhere.",
    ...
    notes=N("question","existing","Calmer opens on gender and echoes it later. Kept in the wish list because Alex flagged it as interesting; the echo stays light because a round-2 tester read Calmer's gendered copy as 'given to every woman'. Open for review.",
...
add(id="goals_metrics", type="goalsMetrics", phase=3, title=["Here's what our","members are saying:"], metrics=[{"goal":"stress","text":"95% report less stress"},{"goal":"mood","text":"92% report improved mood"},{"goal":"sleep","text":"82% report better sleep"},{"goal":"focus","text":"75% report increased focus"}],
    ...
    notes=N("goalsMetrics","copy","Existing card with a dated source line. Sourced stats were March's strongest confidence builder; ours are unsourced.", evidence="March B4/B5"))
...
add(id="stress_echo", type="text", phase=3, branch=STRESS, tap=True, kicker="You're not alone", title=["Most people come to","Balance for exactly this."],
    ...
    notes=N("text (with interpolation)","new","Validation beat that reads the answer back within seconds. Today the flow asks 8 to 10 questions and reflects none of them until the loading screen.", calmer="#8 (validation interstitial)", evidence="Synthesis #1; Alex 5:00", fills=["61%: pull the real share from onboarding answers"]))
...
add(id="mood_2", type="question", phase=3, branch=MOOD, questionId="happiest_around", title=["Who do you usually","feel happiest around?"], subtitle="It shapes the examples in your sessions.",
    ...
    notes=N("question","copy","Existing card plus a reason line."))
add(id="mood_echo", type="text", phase=3, branch=MOOD, tap=True, kicker="You're not alone", title=["Most members start","Balance on a hard week."], body="{{a.happiest_around==='myself'?'Time to yourself is where you recharge, so your sessions will protect it.':'You feel happiest around '+H.lower(L.happiest_around||'people you love')+', so your sessions will help you show up for them.'}} [92%] of members report improved mood.",
    notes=N("text (with interpolation)","new","Validation echo for the mood path.", calmer="#8", evidence="Synthesis #1"))
add(id="mood_science", type="primer", phase=3, branch=MOOD, title=["A short daily practice","lifts mood over","a few weeks."], body="A 2023 review of mindfulness programs in adults found consistent improvements in mood and psychological distress after 8 weeks.",
    cite="Galante et al., Nature Mental Health, 2023. [Confirm with Anna.] Results vary from person to person. Balance is not a substitute for professional care.",
    notes=N("goalMeditationPrimer","unused","Cited education beat for the mood path.", calmer="#9 to #11", fills=["Confirm the Galante 2023 summary wording with Anna"]))
...
add(id="focus_echo", type="text", phase=3, branch=FOCUS, tap=True, kicker="You're not alone", title=["Attention wanders.","That's what it does."], body="You said {{H.lower(L.most_distracting)}} pull{{a.most_distracting==='thoughts'||a.most_distracting==='people'?'':'s'}} you away most. Meditation is practice at noticing that and coming back, and [75%] of members report better focus.",
...
add(id="coaches", type="coaches", phase=3, title=["Your coaches are","Ofosu and Leah."], bios=[{"name":"Ofosu","text":"[One credential line, e.g. has taught meditation for 20 years]"},{"name":"Leah","text":"[One credential line]"}],
    ...
    notes=N("list (2 items with image, title, subtitle)","unused","The decided 'made by humans' screen (angle 1, Sep 4). Showcase, not chooser; no 'AI' in the copy. Headline register to be matched in review.",
      evidence="handcrafted-coaches-screen-angles.md; 3 competitors lean on teacher credibility", fills=["Cindy's approved bios for the credential lines","Headline: question-and-statement register"]))
...
add(id="no_judgment", type="text", phase=4, tap=True, title=["The habits you already","have count for a lot."], body="{{a.schedule==='packed'||a.schedule==='busy'?'Your days are '+H.lower(L.schedule)+', so your first sessions run 10 minutes or less, with 5-minute versions for the days that get away from you.':'You have '+H.lower(L.schedule||'some room to breathe')+', so your first sessions run 10 minutes, with shorter ones for the days that fill up.'}} No pressure, no streak to protect.",
...
# ---------------- Part 5: Your program ----------------
add(id="proof", type="userReview", phase=5, kicker="Did you know?", title=["Balance has helped","<b>{{L.age_count}}</b> people","your age."], laurels=BADGES, reviews=REVIEWS, cite="As of [January 2026]. [Source.] Reviews verbatim from the App Store.", cta="Continue",
    notes=N("ageMetrics + userReview","new","One proof screen instead of two: the age-specific stat (March's strongest confidence builder) as the headline, badges, and real reviews. In the deck these are two templates (ageMetrics; userReview shows one quote per card).", calmer="#33 (social proof carousel)", evidence="Alex: combine the reviews screen with the age stat; March B4 (12 of 18); Alex 7:28", fills=["3 verified App Store reviews, verbatim, with first names","Current date and source for the age count"]))
...
add(id="chart", type="chart", phase=5, title=["Feel better, faster,","with Balance."], weeks=["Today","Week 1","Week 3","Week 6"], withLabel="With Balance", aloneLabel="On your own",
    ...
    notes=N("textImage (static image per goal)","new","The progress graph Alex called 'so important': on your own vs with Balance, never 'Balance Premium'. A static image per goal fits textImage.", calmer="#38 (projection graph)", evidence="Alex 8:35 to 9:00", fills=["Survey figures behind the curve","Copy pass against the web funnel's Before/Day 2/Day 30 timeline"]))
...
add(id="commitment", type="commitment", phase=5, questionId="commitment", title=["How many days a week","feels realistic?"], subtitle="Consistency matters more than length. 10 minutes counts.", cta="Set my goal",
    ...
    notes=N("question","existing","Commitment device right before the reminder and paywall. Plain single-select; the praise line is a per-answer subtitle swap.", calmer="#41 (commitment pledge)", evidence="Synthesis #4; Alex 9:57", fills=["The real number for the footer, or drop the line"]))
...
add(id="push", type="pushOptIn", phase=5, questionId="push", title=["Get a reminder at","{{a.goal_sleep==='yes'?'your target bedtime':'your chosen time'}}"], body="{{a.goal_sleep==='yes'?'Reminders help you set a consistent sleep schedule.':'Reminders help you build better habits.'}}", foot="[Members who turn on reminders are N times more likely to keep going in week 1.]", cta="Continue",
    notes=N("pushOptIn","copy","Existing OS permission wrapper with a real number in the subtext.", evidence="Benchmark rule 8", fills=["The real reminder number, or drop the line"]))
...
add(id="paywall", type="paywall", phase=5, headlines={"stress":"Reduce daily stress and anxiety","sleep":"Fall asleep faster, wake up rested","focus":"Sharpen your focus","mood":"Feel steadier every day","default":"Reduce daily stress and anxiety"},
    notes=N("Superwall post_sign_up","superwall","Terminal screen: the paywall as recorded Aug 16 (carousel). Alex: the live one is now the ReciMe version, so swap the mock if this is used for testing. Research ideas live here as notes: reminder-promise headline with a Day 0/5/7 timeline, an on-paywall 'how do I cancel' answer, echo bullets built from the picks.", calmer="#43 (trial timeline paywall, not copied)", evidence="Alex 10:05 to 10:18; round 2 A3", fills=["Swap for the ReciMe paywall design","Sleep/focus/mood headlines from the live paywall"]))
...
by["sleep_echo"].update({"body":"Stress, discomfort and a mind that won't switch off are the usual reasons. [82%] of members report better sleep, and the first thing your program works on is the wind-down that gets you there."})
...
by["mood_echo"].update({"body":"Whether you recharge alone or with the people you love, your sessions will help you protect that time. [92%] of members report improved mood."})
...
by["focus_echo"].update({"body":"Thoughts, screens, noise, other people: whatever pulls you away, meditation is practice at noticing it and coming back. [75%] of members report better focus."})
...
age_card = {"id":"age_metrics","type":"ageMetrics","phase":5,"kicker":"Did you know?","title":["Balance has helped","<b>{{L.age_count}}</b> people your age","improve mental wellness."],"cite":"As of [January 2026]. [Source.]",
  "notes":N("ageMetrics","copy","The age-specific stat, kept as its own card (the template computes the count). Needs a current date and a source.", evidence="March B4/B5")}
review_card = {"id":"proof","type":"userReview","phase":5,"title":["Members like you","say it works."],"laurels":BADGES,"reviews":REVIEWS[:1],"cite":"Verbatim from the App Store.","cta":"Continue",
  "notes":N("userReview (one quote + image per card)","unused","One testimonial card. userReview shows a single quote and an asset, so the carousel is one card here (or several cards).", calmer="#33", loss="The combined age-stat headline and the 3-review carousel; badges depend on the asset.", fills=["1 verified App Store review, verbatim"])}
