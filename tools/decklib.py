...
PRINCIPLES = {
 "arrival": {"name":"Match the ad they tapped","text":"Most new users arrive from a Meta ad about stress or anxiety and have never used a meditation app. Speak to the reason they tapped before asking anything personal.","source":"HDYHAU Sep 4: Meta is 40% of respondents at 6.6% trial start vs 13.9% overall; round 2 A2 (newcomers reached a paywall not knowing what the app does)"},
 "proof-early": {"name":"Lots of proof before the trial ask","text":"Awards, ratings, member counts and reviews appear while the user is still deciding whether to give us 3 minutes, not only at the paywall.","source":"March A3 (the store listing was the strongest pre-app trust signal); Calmer #1; Insight Timer's 5 proof placements"},
 "payoff": {"name":"Every question pays off","text":"Each question says why we ask, and the answer visibly changes something within 2 screens.","source":"Growth Gems principle 2; 23-app benchmark rule 2 (reason in the header); rule 3 (result points back to what you said)"},
 "echo": {"name":"Say their answers back","text":"Read the user's own answers back to them. Personalization the user cannot see is not personalization to them.","source":"Synthesis #1 (answer echo, the biggest gap vs the field); round 2 A4 (a named result is where 'it understood me' landed); Alex 5:00 ('a major need')"},
 "rhythm": {"name":"Ask, then give back","text":"The question rhythm: never 2 question blocks in a row. After 1 or 2 questions, pay the user back with a validation, a fact or a look at the product.","source":"Calmer's spine (15 questions, ~20 interstitials); synthesis #2; round 2: 3 of 4 Calmer testers named the explanations unprompted"},
 "sourced": {"name":"Real numbers with a source","text":"Every number carries a source and a date, and results-vary language where it applies. No 'clinically proven', no unsourced stats.","source":"March B4/B5 (age-specific stats were the strongest confidence builder; ours were dated and unsourced); Anna's citation set; Balance whitepaper 2025"},
 "felt-value": {"name":"Show the product before the paywall","text":"The paywall arriving before any sense of the product was the abandonment trigger in March. Previews of real sections and a named first session stand in for felt value.","source":"March D1 (12 of 18) and F1 (5 of 6 newcomers); round 2 A5 (content access resolved the try-first objection); synthesis #10; paid-UA field read Sep 5 (ReciMe has the user import a real recipe before its paywall)"},
 "humans": {"name":"Real coaches, shown not claimed","text":"Faces, voices and how the sessions get made answer the 'is this AI' worry without using the word, which plants the thought.","source":"Cindy (Sep 2024); coaches prior art; App Store review 'Love that they use real voices and not AI'; 3 competitors sell teacher credibility"},
 "reassure": {"name":"A privacy line beside personal questions","text":"A concrete, checkable line next to the field it protects ('never shown to anyone') builds more trust than any general promise.","source":"23-app benchmark rule 4 (Strava, Gentler Streak: highest trust ratings in the set); the deck had zero reassurance copy"},
 "no-deficit": {"name":"No scare copy","text":"Validate what the user said, then teach. No presupposed diagnosis, always a 'Not sure'. Calmer's agitate-then-absolve structure is used; its claims are not.","source":"Balance voice (compassionate, grounded); synthesis #9; Calmer's dark patterns documented, not copied"},
 "commit": {"name":"A small commitment before the trial","text":"A realistic practice goal and a reminder time right before the paywall turn intent into a plan, so the trial feels like a decision already half made.","source":"Synthesis #4 (Insight Timer's praise ladder, cheapest high-leverage steal); Calmer #41; CYTR +24.7% trial starts"},
 "honest-result": {"name":"A profile built only from real answers","text":"The profile reflects what the user actually said. No invented sub-scores, no manufactured headroom, no clinical framing.","source":"Round 2 P18 (Calmer scored his healthy diet as Fair and he thanked it); A4; 'not a clinical assessment' line"},
 "keep-what-works": {"name":"Keep what already tests well","text":"The goal screens, the reminder flow, the trial-reminder chooser and the live paywall stay as they are. This is a constraint as much as a principle: no first test would remove the goal screens or change the reminder flow, so both versions keep them.","source":"Alex 4:35 ('keep our goal screens as they exist') and Sep 5 ('constraint-driven more than we know this is good'); prior-art census (wins came from reordering and framing, not new content); CYTR and ReciMe are baseline wins"},
...
def lint(deck, constrained=False, allowed=None, name_ok=None):
    ...
    for c in deck["cards"]:
        ...
        if constrained:
            ...
            if bad: errs.append(f"{c['id']}: answers written into copy in the constrained deck: {bad[:2]}")
...
def finish(decks, allowed, name_ok, check=True):
    ...
    if ok:
        ...
        for name in ("index.html","insight-timer.html"):
